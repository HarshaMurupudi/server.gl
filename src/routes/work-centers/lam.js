...
router.get("/lam/jobsByWorkCenter/:workCenterName", async (req, res) => {
  try {
      ...
      jobIds.length > 0
        ? await glDB.query(
          ...
          FROM (
            ...
            LEFT JOIN
            (SELECT * FROM [General_Label].[dbo].[LAM_Notes] ) AS t3 
            ON 
              jo.Job = t3.Job
...
router.get("/lam/jobs/open/:workCenterName", async (req, res) => {
  try {
    const { workCenterName } = req.params;
...
router.get("/lam/jobs/open/:workCenterName/now-at", async (req, res) => {
  try {
    const { workCenterName } = req.params;
