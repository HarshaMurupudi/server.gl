...
router.get("/inspection/jobsByWorkCenter/:workCenterName", async (req, res) => {
  try {
      ...
      jobIds.length > 0
        ? await glDB.query(
          ...
          FROM (
            ...
            LEFT JOIN
            (SELECT * FROM [General_Label].[dbo].[Inspection_Notes] ) AS t3 
            ON 
              jo.Job = t3.Job
...
router.get("/inspection/jobs/open/:workCenterName", async (req, res) => {
  try {
    const { workCenterName } = req.params;
...
router.get("/inspection/jobs/open/:workCenterName/now-at", async (req, res) => {
  try {
    const { workCenterName } = req.params;
