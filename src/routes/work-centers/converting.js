...
router.get('/converting/jobsByWorkCenter/:workCenterName', async (req, res) => {
  try {
      ...
      const fJobs = await glDB.query(
        ...
        SELECT * FROM (
          ...
          FROM (
            ...
            LEFT JOIN
            (SELECT * FROM [General_Label].[dbo].[Converting_Notes] ) AS t3 
            ON 
              jo.Job = t3.Job
...
router.get('/converting/jobs/open/:workCenterName/now-at', async (req, res) => {
  try {
    const { workCenterName } = req.params;
