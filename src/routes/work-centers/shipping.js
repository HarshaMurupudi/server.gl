...
router.get("/shipping/jobsByWorkCenter/:workCenterName", async (req, res) => {
  try {
      ...
      jobIds.length > 0
        ? await glDB.query(
          ...
          FROM (
            ...
            LEFT JOIN
            (SELECT * FROM [General_Label].[dbo].[Shipping_Notes] ) AS t3 
            ON 
              jo.Job = t3.Job
...
router.get("/shipping/jobs/open/:workCenterName", async (req, res) => {
  try {
    const { workCenterName } = req.params;
...
router.get("/shipping/jobs/open/:workCenterName/now-at", async (req, res) => {
  try {
    const { workCenterName } = req.params;
